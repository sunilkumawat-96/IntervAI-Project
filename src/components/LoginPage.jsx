...
const LoginPage = () => {
  ...
  const handleLogin = (e) => {
    ...
    // Fetch existing user or create a mockup profile based on the login email
    const existingUser = JSON.parse(localStorage.getItem('intervai_user')) || {};
    const defaultName = email.split('@')[0] || 'User'; // Extract name from email if missing
    
    const userData = {
      name: existingUser.name || defaultName,
      email: email,
      role: existingUser.role || 'Software Engineer',
