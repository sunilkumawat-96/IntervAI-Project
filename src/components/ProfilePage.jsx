...
const ProfilePage = () => {
  ...
  // Initialize state directly from localStorage if it exists, otherwise use fallback data
  const [profileDetails, setProfileDetails] = useState(() => {
    const savedUser = localStorage.getItem('intervai_user');
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-1 space-y-6">
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 border border-slate-200 dark:border-slate-800 shadow-sm text-center">
              ...
              <button onClick={toggleEditMode} className={`w-full py-3 font-bold rounded-xl transition-colors text-sm shadow-inner ${isEditing ? 'bg-blue-600 hover:bg-blue-700 text-white' : 'bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300'}`}>
